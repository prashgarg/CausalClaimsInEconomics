...
def load_stage1_rows(stage1_path: Path) -> List[Dict[str, Any]]:
    rows = read_jsonl(stage1_path)
    if not rows:
...
def build_requests(
    ...
    s1_rows = load_stage1_rows(stage1_path)
    ...
    for row in s1_rows:
        ...
        causal_identification = str(row.get("causal_identification", ""))
        causal_claim = str(row.get("causal_claim", ""))
        ...
        for j in range(1, iters + 1):
            ...
            user_prompt = (
                ...
                .replace("<<STAGE1_CAUSAL_IDENTIFICATION>>", causal_identification)
                .replace("<<STAGE1_CAUSAL_CLAIM>>", causal_claim)
                .replace("<<STAGE1_CLAIM_SNIPPETS>>", snippets_text)
            )
            body = {
                "model": stg["model"],
                "messages": [
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Stage 2 edge extraction runner.")
    ...
    if not stage1_path.exists():
        print(f"[STAGE2] Missing Stage 1 file: {stage1_path}", file=sys.stderr)
        return 1

    try:
        requests, out_dir = build_requests(config_path, stage1_path)
    except Exception as e:  # noqa: BLE001
        print(f"[STAGE2] Failed to build requests: {e}", file=sys.stderr)
        return 1

    if args.max_requests is not None:
        requests = requests[: max(0, args.max_requests)]

    req_path = out_dir / "stage2_requests.jsonl"
    write_jsonl(req_path, requests)
    print(f"[STAGE2] Wrote request file: {req_path}")

    if not args.execute:
        print("[STAGE2] Dry mode complete. Use --execute to call the API.")
        return 0

    try:
        client = get_openai_client()
    except Exception as e:  # noqa: BLE001
        print(f"[STAGE2] {e}", file=sys.stderr)
        return 1
    ...
    for idx, req in enumerate(requests, start=1):
        ...
        print(f"[STAGE2] Executing {idx}/{len(requests)}: {cid}")
        try:
    ...
    raw_path = out_dir / "stage2_raw_responses.jsonl"
    edges_path = out_dir / "stage2_edges_raw.jsonl"
    write_jsonl(raw_path, raw_rows)
    write_jsonl(edges_path, edge_rows)
    print(f"[STAGE2] Wrote raw responses: {raw_path}")
    print(f"[STAGE2] Wrote flattened edges: {edges_path}")
    return 0
